...
class GroundOrientationEstimator:
    ...
    def reset(self):
        # self._base_orientation_matrix_zero_yaw = self.get_zero_yaw_rotation_matrix()
        # self._foot_position_history = np.array(
        #     self._base_orientation_matrix_zero_yaw.dot(self._robot.foot_pos_b_np[self._env_ids].T).T)
        ...
        # self._base_orientation_matrix_zero_yaw = np.eye(3)
        self._ground_normal_vec = np.array([0., 0., 1.])

    def get_zero_yaw_rotation_matrix(self):
        base_rpy = self._robot.base_rpy_w2b_np[self._env_ids]
        return roty(base_rpy[1]).dot(rotx(base_rpy[0]))
    ...
    def update(self):
        ...
        # self._base_orientation_matrix_zero_yaw = self.get_zero_yaw_rotation_matrix()
        # base_vel = self._base_orientation_matrix_zero_yaw.dot(self._robot.base_lin_vel_b_np[self._env_ids])
        base_vel = self._robot.base_lin_vel_w_np[self._env_ids]
        ...
        for idx, contact in enumerate(foot_contacts):
            if contact:
                # self._foot_position_history[idx] = self._base_orientation_matrix_zero_yaw.dot(new_foot_positions[idx])
                self._foot_position_history[idx] = self._robot.foot_pos_w_np[self._env_ids][idx]
            else:
                self._foot_position_history[idx] -= base_vel * dt
        ...
        # robot_x = self._base_orientation_matrix_zero_yaw[:, 0]
        robot_x = self._robot.base_rot_mat_w2b_np[self._env_ids][:, 0]
    ...
    @property
    def base_orientation_matrix_zero_yaw(self):
        return self._base_orientation_matrix_zero_yaw
