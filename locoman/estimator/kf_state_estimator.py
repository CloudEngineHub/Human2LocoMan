...
class KFStateEstimator:
  ...
  def _get_velocity_and_height_observation(self):
    ...
    # foot_positions = self._robot.foot_positions_in_base_frame
    foot_positions = self._robot.foot_pos_b_np[0]
    foot_velocities = self._robot.foot_vel_b_np[0]
    # ang_vel_cross = convert_to_skew_symmetric(self._robot.base_angular_velocity_body_frame)
  ...
  def set_robot_base_state(self):
    # self._robot._base_pos_w[:] = torch.tensor(self._estimated_position, dtype=torch.float, device=self._robot._device)
    self._robot._base_pos_w[:, 2] = self._estimated_position[2]
    self._base_rpy_w2b[0] = self._robot._raw_state.imu.rpy[0]
    self._base_rpy_w2b[1] = self._robot._raw_state.imu.rpy[1]
    self._robot._base_rpy_w2b[:] = torch.tensor(self._base_rpy_w2b, dtype=torch.float, device=self._robot._device)
    self._robot._base_rot_mat_w2b[:] = torch.tensor(rpy_to_rot_mat(self._base_rpy_w2b), dtype=torch.float, device=self._robot._device)
    self._robot._base_rot_mat_b2w[:] = self._robot._base_rot_mat_w2b.transpose(-2, -1)
    ...
    self._robot._base_lin_vel_w[:] = torch.tensor(self._zeroyaw_rot_mat_zy2w.dot(self._estimated_velocity.reshape(-1, 1)).flatten(), dtype=torch.float, device=self._robot._device)
    self._robot._base_lin_vel_b[:] = torch.matmul(self._robot._base_rot_mat_b2w, self._robot._base_lin_vel_w.reshape(-1, 1)).flatten()
    self._robot._base_ang_vel_w[:] = torch.tensor(self._zeroyaw_rot_mat_zy2w.dot(self._angular_velocity.reshape(-1, 1)).flatten(), dtype=torch.float, device=self._robot._device)
    self._robot._base_ang_vel_b[:] = torch.matmul(self._robot._base_rot_mat_b2w, self._robot._base_ang_vel_w.reshape(-1, 1)).flatten()
