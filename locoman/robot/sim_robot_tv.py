...
class SimRobot(BaseRobot):
    def __init__(self, cfg: Cfg, sim, viewer):
        ...
        self.sim_view_publisher = rospy.Publisher(Cfg.teleoperation.teleop_view_topic, Image, queue_size=1)
    ...
    def _create_envs(self):
        ...
        self.fpv_height = self._cfg.teleoperation.fpv_height
        self.fpv_width = self._cfg.teleoperation.fpv_width
        self._cameras = []
        ...
        for i in range(self._num_envs):
            ...
            camera_props.width = self.fpv_width
            camera_props.height = self.fpv_height
            ...
            self._gym.set_camera_location(camera_handle, env_handle, gymapi.Vec3(0.6, -0.6, 0.45), gymapi.Vec3(0, 0, 0.25))
    ...
    def step(self, action: MotorCommand, motor_control_mode: MotorControlMode = None, gripper_cmd=True):
        ...
        proprio_states = np.concatenate([body_pos, body_rpy, eef_pos, eef_rpy, gripper_angles, joint_pos, joint_vel])
        ...
        ros_image = ros_numpy.msgify(Image, rgb_image, encoding='bgr8')
        self.sim_view_publisher.publish(ros_image)
        show_img = True
        # if show_img:
        #     if self.realtime_img is None:
        #         ax = plt.subplot()
    ...
    def _action_to_torque(self, action: MotorCommand, motor_control_mode: MotorControlMode = None):
        ...
        if motor_control_mode == MotorControlMode.POSITION:
            ...
            # print('gripper 0 pos', self._joint_pos[0, 3:6])
            # print('gripper 1 pos', self._joint_pos[0, 9:12])
            # print('gripper 0 des pos', action.desired_position[0, 3:6])
            # print('gripper 1 des pos', action.desired_position[0, 9:12])
            # print('gripper 0 vel', self._joint_vel[0, 3:6])
            # print('gripper 1 vel', self._joint_vel[0, 9:12])
            # print('gripper 0 des vel', action.desired_velocity[0, 3:6])
            # print('gripper 1 des vel', action.desired_velocity[0, 9:12])
            # print('gripper 0 kp', action.kp[0, 3:6])
            # print('gripper 1 kp', action.kp[0, 9:12])
            # print('gripper 0 des torque', action.desired_extra_torque[0, 3:6])
            # print('gripper 1 des torque', action.desired_extra_torque[0, 9:12])
            # print('gripper 0 final torque', self._torques[0, 3:6])
            # print('gripper 1 final torque', self._torques[0, 9:12])
            # if torch.max(abs(self._torques[0, :6])) > 3:
