...
class RaibertSwingLegPlanner:
    ...
    def __init__(self,
        ...
        self._hip_positions_body_frame = np.array([[0.1881, -0.12675, 0],
                                                   [0.1881, 0.12675, 0],
                                                   [-0.1881, -0.12675, 0],
                                                   [-0.1881, 0.12675, 0]])
    ...
    def reset(self):
        ...
        # self._phase_switch_foot_local_position = self._ground_estimator.base_orientation_matrix_zero_yaw.dot(self._robot.foot_pos_b_np[self._env_ids].T).T
    ...
    def update(self):
        ...
        # rot_mat = self._ground_estimator.base_orientation_matrix_zero_yaw
    ...
    def get_desired_foot_positions(self):
        ...
        # rot_mat = self._ground_estimator.base_orientation_matrix_zero_yaw
        # com_velocity_body_frame = self._robot.base_lin_vel_b_np[self._env_ids]
        # com_velocity = rot_mat.dot(com_velocity_body_frame)
        com_vel_zy = self._robot.base_lin_vel_w_np[self._env_ids]

        # angular_vel_body_frame = np.array(self._robot.base_ang_vel_b_np[self._env_ids])
        # hip_positions = rot_mat.dot(self._hip_positions_body_frame.T).T
        hip_offset_zy = self._robot.base_rot_mat_w2b_np[self._env_ids].dot(self._hip_positions_body_frame.T).T
        ...
        for leg_id, leg_state in enumerate(self._gait_generator.leg_state):
            ...
            # hip_velocity = com_velocity +rot_mat.dot(np.cross(angular_vel_body_frame, self._hip_positions_body_frame[leg_id]))
            hip_velocity = com_vel_zy + self._robot.base_rot_mat_w2b_np[self._env_ids].dot(np.cross(self._robot.base_ang_vel_b_np[self._env_ids], self._hip_positions_body_frame[leg_id]))
            # Mid air position
